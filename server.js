...
app.post('/deploy', async (req, res) => {
  try {
    ...
    if (!repo_url || !image_name || !registry || !app_name || !registry_auth) {
    ...
    await createKanikoJob(jobId, { repo_url, image_name, registry, registry_auth });
...
async function createKanikoJob(jobId, { repo_url, image_name, registry, registry_auth }) {
  const job = {
    apiVersion: 'batch/v1',
    kind: 'Job',
    metadata: {
